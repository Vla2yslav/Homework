...
public class Main {
    public static void main(String[] args) {
        ...
        orders.stream().sorted(new Compare());
        orders.stream().sorted(new comparecity());
        orders.stream().sorted(new compare3());

        Predicate<Order> price = p -> p.getPrice() > 1500;
        Predicate<Order> surname = s -> s.getUser().getLastName().equalsIgnoreCase("Petrov");
        Predicate<Order> currency = c -> c.getCurrency().equals(Currency.UAH) || c.getCurrency().equals(Currency.EUR);

        orders.stream().distinct().collect(Collectors.toList())
                .forEach(System.out::println);

        orders.stream().filter(price)
                .forEach(System.out::println);
